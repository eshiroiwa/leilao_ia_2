...
def test_geocodificar_texto_livre_chama_cached():
    with patch.object(geo, "_geocode_cached", return_value=(-20.0, -43.0)) as m:
...
def test_rua_sem_numero_tenta_bairro_antes_da_rua():
    ...
    with patch.object(geo, "_geocode_structured_cached", m_struct):
        with patch.object(geo, "_geocode_cached", side_effect=moema_primeiro):
            r = geo.geocodificar_endereco(
