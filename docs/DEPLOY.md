...
O pacote instalável contém **apenas** `leilao_ia_v2` (incluindo `leilao_ia_v2.fc_search`). As dependências de aplicação vêm do `requirements.txt`.

## Configuração

1. Crie `.env` na **raiz do repositório** (o Streamlit em `app_assistente_ingestao.py` carrega `../.env` em relação ao ficheiro; o CLI de `fc_search` usa a raiz).
...
## CLI — complemento Firecrawl Search (comparáveis)
